...
async function not_found_page(res_data) 
{
    try {
        ...
        let page_header = template_header.replace('{{ page_title }}', '404 | Not Found').replace('{{ description }}', 'Not Found page');
...
async function dashboard(method, res_data) 
{
    ...
    try {
        ...
        let page_header = template_header.replace('{{ page_title }}', 'Dashboard').replace('{{ description }}', 'Checks Dashboard');
        res_data.content_type = 'text/html';
...
async function check_create(method, res_data) {
    ...
    try {
        ...
        let page_header = template_header.replace('{{ page_title }}', 'Create Check').replace('{{ description }}', 'Create a check');
        res_data.content_type = 'text/html';
        res_data.status_code = 200;
        res_data.payload = page_header + page_content + template_footer;
    } catch (error) {
...
async function check_edit(method, res_data) {
    ...
    try {
        ...
        let page_header = template_header.replace('{{ page_title }}', 'Edit Check').replace('{{ description }}', 'Edit a check');
        res_data.content_type = 'text/html';
        res_data.status_code = 200;
        res_data.payload = page_header + page_content + template_footer;
    } catch (error) {
...
async function checks_JSON(method, res_data) {
    ...
    try {
        ...
        let page_header = template_header.replace('{{ page_title }}', 'Checks JSON').replace('{{ description }}', 'Visualize the checks JSON');
        res_data.content_type = 'text/html';
        res_data.status_code = 200;
        res_data.payload = page_header + page_content + template_footer;
    } catch (error) {
