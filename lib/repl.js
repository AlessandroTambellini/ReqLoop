...
/* I think having a REPL to show and filter the checks without opening the web interface can be handy. */
...
function init_main_REPL() {
    ...
    repl_server.on('exit', async () => {
        ...
        await data.write_checks_to_disk();
        init_secondary_REPL();
    });
}
...
{
    console.log('Type \'open repl\' to open again the main REPL.');
    const eval_open_main_REPL = (cmd, context, filename, callback) => {
        ...
        if (tokens[0].toLowerCase() === 'open' && tokens[1].toLowerCase() === 'repl') {
            callback(null);
