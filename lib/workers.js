...
function is_check_valid(check_id, check_obj) 
{
    let url_obj;
    try {
    ...
    const protocol = url_obj.protocol.split(':')[0] === 'http' ? http : https;
    const req = protocol.request(url_obj, options);
...
function start_background_workers() 
{
    ...
    const perform_checks = async () => {
        ...
        if (res.Error) {
            ...
            {
                if (is_check_valid(check_id, check_obj)) {
                    // Save to .log file
